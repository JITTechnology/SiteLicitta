...
$segments = explode('/', trim($uri, '/')); // Separa a URL em segmentos
$slug = end($segments); // Pega o último segmento, que é o slug da tag

// Tenta pegar a tag pelo slug
$tag_object = get_term_by('slug', $slug, 'post_tag'); 

$tag_name = "Tag não encontrada."; // Valor padrão

if ($tag_object) {
    $tag_name = $tag_object->name; // Se encontrar, pega o nome da tag
}

$endpoint = home_url().'/blog/tema/' . $slug;
$title = 'Blog - Tag ' . $tag_name;
$description = 'Confira as melhores dicas, curiosidades e tudo o que você precisa saber sobre '.$tag_name.' com a OK Intercâmbio, especialista em intercâmbios e educação internacional.';
?>
            ...
            <div class="container">
                <div class="row">
                    <div class="col-md-8">
                        ...
                        ?>
                        <h4 class="title_sessao">Postagens sobre
                            <?=$tag_name?></h4>
					...
					<div class="col-md-4">
						...
						<div id="section-ultimas">
							<div class="col-sm-12">
                                <?php
                            $args = array(
                                'posts_per_page' => ceil($totalPosts / 2) * 3,
                                'meta_key' => 'views',
                                'orderby' => array(
                                    'meta_value_num' => 'DESC',
                                    'date' => 'DESC'
